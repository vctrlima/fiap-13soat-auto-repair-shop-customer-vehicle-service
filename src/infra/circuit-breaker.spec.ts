...
describe("DatabaseCircuitBreaker", () => {
  ...
  it("should trip to OPEN when failure rate exceeds threshold", async () => {
    ...
    // 3 failures, 0 successes → 100% failure rate
    for (let i = 0; i < 3; i++) {
      await cb.execute(() => Promise.reject(new Error("fail"))).catch(() => {});
    }
  ...
  it("should reset rolling window counters when window expires", async () => {
    ...
    // Accumulate 2 failures within the window
    await cb.execute(() => Promise.reject(new Error("fail"))).catch(() => {});
    await cb.execute(() => Promise.reject(new Error("fail"))).catch(() => {});
    ...
    // After window reset, this 1 failure alone is below volumeThreshold of 3
    await cb.execute(() => Promise.reject(new Error("fail"))).catch(() => {});
    expect(cb.currentState).toBe("CLOSED");
  });
